...
def evalImage(goldMask=None, segmMask=None, thresholdLevels= thresholdLevels):
		precision = 0.0
		selectivity = 0.0
		spec = 0.0
		...
		orig_precision = np.mean(TP / (TP+FP))
	...
	#  print("FP",FP)
 #  print("asd",uvGT)
	#Number of Patients minus True Positives = False Negatives
	 # selectivity = np.mean(TN / nofGT)
	 	# recall = sensitiivity
		
		sensitivity = np.mean(TP / nofGT)
		f1_score = 2 * (orig_precision * sensitivity) / (orig_precision + sensitivity)
		#print(sensitivity)
...
def eval_dir(gold_dir, segm_dir):
	...
	for file in os.listdir(segm_dir):
		if file.endswith(".tiff") or  file.endswith(".png"):
			...
			value, sensitivity, f1  = evalImage(gold, segm)
			values.append(value)
			sens.append(sensitivity)
			f1_arr.append(f1)
		 #  print(basename + '\t' + "{:1.3f}".format(value))
	print("Iou th", thresholdLevels)
	valarr = np.array(values)
	senarr= np.array(sens)
