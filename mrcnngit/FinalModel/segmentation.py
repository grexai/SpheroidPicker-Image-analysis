...
class Segmentation:
    ...
    '''
    def __init__(self, pModelPath, pConfidence=0.5, pNMSThreshold = 0.35):
    ...
    def Segment(self, pImage, pPaddingRatio=0.0, pDilationSElem=None, pCavityFilling=False, pPredictSize=None):
        ...
        masks = r['masks']
        scores = r['scores']
        print(masks.shape[1],"1:2",masks.shape[2],"image",image.shape[0],image.shape[1])
        ...
        if pCavityFilling:
            for i in range(count):
                ...
                temp,contours = cv2.findContours(temp, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
#                temp,_ = cv2.threshold(,127,255,cv2.THRESH_BINARY)
                masks[:, :, i] = cv2.bitwise_not(contours)
#            for i in range(count):
...
method = Segmentation(pModelPath=modelPath, pConfidence=confidence, pNMSThreshold=nmsThresh)
